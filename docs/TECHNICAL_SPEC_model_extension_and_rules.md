...
**Target files:** `app/models/monday_raw.py`, `app/models/normalised.py` (or equivalent), `app/audits/rules.py`
...
2. Exact field-level additions to the normalised model
...
## 3. Normalised model extensions

The normalised model is where cross-board rules (like Rule 1) become answerable in one place instead of requiring a full-account join at rule-evaluation time.

### 3.1 Extend: `NormalisedColumn`
...
Computed as `len([i for i in board.items if i.group_id == group.id])` at normalisation time — this is the recommended approach per 2.3 unless the raw export genuinely can't cheaply recompute it.

### 3.3 New: `NormalisedAutomation`

```python
class NormalisedAutomation(BaseModel):
    board_id: str
    board_name: str                # denormalised for report readability
    trigger_type: AutomationTriggerType
    action_type: AutomationActionType
    target_board_id: Optional[str] = None
    target_board_name: Optional[str] = None   # denormalised; None if same-board or unresolvable
    is_cross_board: bool           # derived: target_board_id is not None and target_board_id != board_id
```

`is_cross_board` should be computed once during normalisation, not recomputed inside the rule — the rule should just filter on it.

### 3.4 Extend: `NormalisedBoard`

```python
class NormalisedBoard(BaseModel):
    # ...existing fields...
    automations: list[NormalisedAutomation] = []
```
...
- **Input:** `NormalisedBoard.automations`
...
- **Input:** all `NormalisedColumn` of type `text` (or long_text) across a board, plus a sample of cell values for that column (item-level data, sampled — not full extraction).
...
- **Severity:** `ADVISORY` — a normalisation suggestion, not a hard blocker.
- **Message template:** `"Column '{column_title}' on '{board_name}' stores structured, delimiter-separated values ({sample_values}) as plain text rather than a relation to another board. Consider modelling this as a board_relation for the migration target."`
...
- **Input:** `NormalisedColumn.is_locked` across a board
...
| 2 | Normaliser extensions (`NormalisedAutomation`, `is_locked`, `item_count`, `is_cross_board` derivation) | Fixture from Section 5 normalises into valid `NormalisedBoard` objects for all 3 boards |
...
1. Does the current exporter (if one exists beyond stubs) have *any* automation data today, even partial? If yes, check its shape against `RawAutomation` before finalising the field names.
...
- **Rules 1, 3, 4: implemented.** See `app/models/monday_raw.py`, `app/models/normalised.py`, `app/normalisers/monday_normaliser.py`, `app/audits/rules.py`, and the regression test in `tests/test_hacksaw_case_study_rules.py`.
...
- Deviations from this spec as implemented, and why:
  ...
  - Fixture top-level board key is `id`, not `board_id` as shown in Section 5 — matches the real sanitiser/normaliser's existing field name (`schema.get("id")`).
  ...
  - Client/exporter/sanitiser (§4.1 API wiring) intentionally not touched this pass — no live API token exists to validate field names against, and automations aren't confirmed to be on the standard `boards` query at all (§4.1's own caveat).
