...
The tool exports privacy-minimised Monday.com technical structure, normalises boards into a platform-neutral schema model, runs deterministic quality checks, and generates local reports. External GPT analysis and ClickUp transmission are disabled in Phase 1.
...
No monday.com token yet? Run `python scripts/demo_hacksaw_run.py` instead of the `export-all` step above — it runs the same pipeline against a stand-in client shaped like a real case study, so you can see `normalise`/`audit`/`report`/the dashboard working end to end without one.

## Dashboard

`streamlit_app.py` is a local, read-only viewer over whatever `normalise`/`audit` already wrote to disk — it never talks to monday.com itself.
