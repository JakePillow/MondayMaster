...
def finding(rule_id, severity, board: NormalisedBoard, text, evidence, rec, object_type="board", object_id=None, object_name=None):
...
def audit_model(model: NormalisedAccount) -> list[AuditFinding]:
    findings: list[AuditFinding] = []
    for b in model.boards:
        title_types = [(c.title.lower(), c.normalised_type) for c in b.columns]
        ...
        if not any(t == "status" for _, t in title_types):
            findings.append(finding("missing_status_column", "high", b, "Board has no status column.", {"column_types": [c.normalised_type for c in b.columns]}, "Define a status workflow before mapping to ClickUp statuses."))
        text_cols = [c.title for c in b.columns if c.normalised_type == "text"]
        if len(text_cols) > 8:
        ...
        for c in b.columns:
            if c.normalised_type == "formula":
                findings.append(finding("formula_risk", "high", b, "Formula column may not migrate directly.", {"column_id": c.id, "title": c.title}, "Document formula logic and rebuild or materialise values.", "column", c.id, c.title))
            if c.normalised_type == "mirror":
            ...
            if c.normalised_type == "connect_board":
