...
from app.privacy.policy import TechnicalDataSanitiser
...
class MondayExporter:
    def __init__(self, client: MondayClient, store: ArtefactStore):
        self.client = client
        self.store = store
        self.sanitiser = TechnicalDataSanitiser()

    def _start_run(self) -> Path:
        run_dir = self.store.new_run_dir()
        self.store.write_json(run_dir / "privacy_manifest.json", self.sanitiser.manifest())
        return run_dir

    def export_account(self) -> Path:
        run_dir = self._start_run()
        self.store.write_json(run_dir / "account.json", self.sanitiser.account(self.client.get_account()))
        return run_dir

    def export_workspaces(self) -> Path:
        run_dir = self._start_run()
        workspaces = [self.sanitiser.workspace(ws) for ws in self.client.get_workspaces()]
        self.store.write_json(run_dir / "workspaces.json", workspaces)
        return run_dir

    def export_boards(self) -> Path:
        run_dir = self._start_run()
        boards = [self.sanitiser.board_index(board) for board in self.client.get_boards()]
    ...
    def export_all(self, sample_items: int = 100) -> Path:
        run_dir = self._start_run()
        self.store.write_json(run_dir / "account.json", self.sanitiser.account(self.client.get_account()))
        # Keep a stable artefact without collecting any user profiles.
        self.store.write_json(run_dir / "users.json", [])
        workspaces = [self.sanitiser.workspace(ws) for ws in self.client.get_workspaces()]
        self.store.write_json(run_dir / "workspaces.json", workspaces)
        raw_boards = self.client.get_boards()
        boards = [self.sanitiser.board_index(board) for board in raw_boards]
        self.store.write_json(run_dir / "boards_index.json", boards)
        for raw_board, board in zip(raw_boards, boards):
            monday_board_id = str(raw_board["id"])
            board_ref = str(board["id"])
            schema = self.sanitiser.board_schema(self.client.get_board_schema(monday_board_id))
            ...
            summary = self.sanitiser.item_sample_summary(items, sample_items)
    ...
    def export_board(self, board_id: str, sample_items: int = 100) -> Path:
        run_dir = self._start_run()
        schema = self.sanitiser.board_schema(self.client.get_board_schema(board_id))
